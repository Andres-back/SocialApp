...
export function CampaignModePage() {
  ...
  return <div>
    ...
    <section className="card mt-6 p-5 md:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">Análisis agregado</p><h2 className="mt-1 font-display text-3xl text-pine-900">Informe de IA de la brigada</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Genera resultados generales, observaciones y recomendaciones a partir de conteos por opción. No se envían nombres, datos demográficos, textos libres ni valores numéricos. En brigadas pequeñas, un conteo puede corresponder a una sola persona.</p></div><div className="flex flex-wrap gap-2 print:hidden"><button className="btn-secondary" disabled={!online || completed === 0 || generatingReport} onClick={() => void generateCampaignReport()}><Sparkles size={17}/>{generatingReport ? 'Generando…' : reports.length ? 'Generar nueva versión' : 'Generar informe'}</button>{reports.length > 0 && <button className="btn-primary" onClick={() => void printReport()}><Printer size={17}/> Imprimir informe</button>}</div></div>
      {completed === 0 && <div className="mt-5 rounded-xl bg-amber-50 p-4 text-sm text-amber-800 print:hidden">Completa un tamizaje para habilitar el informe. Después podrás generarlo con cualquier cantidad de encuestas completadas.</div>}
