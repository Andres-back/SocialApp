...
import { discardSyncIssue, keepLocalAthleteConflict, listSyncIssues, retrySyncIssue, synchronize, type SyncIssue } from '../lib/sync-engine';
...
export function SyncPage() {
  ...
  async function keepLocal(issue: SyncIssue) {
    if (!window.confirm('¿Conservar tus cambios en los campos señalados? Se combinarán con los demás datos de la versión más reciente.')) return;
    setRunning(true); setMessage('');
    try {
      await keepLocalAthleteConflict(issue.mutationId);
      const result = await synchronize();
      setMessage(result.conflicts === 0 ? 'Tus cambios se combinaron con la versión más reciente.' : 'Todavía hay un conflicto que requiere revisión.');
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'No fue posible resolver el conflicto.');
    } finally {
      setRunning(false); refresh(); void refreshIssues();
    }
  }
