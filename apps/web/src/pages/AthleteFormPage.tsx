...
const schema = z.object({
  internalCode: z.string().trim().min(2, 'Escribe el código interno.').max(40),
  ...
  lastNames: z.string().trim().min(2, 'Escribe los apellidos.').max(120),
  ...
  sportsProgramId: z.string().uuid('Selecciona el programa.'),
  sportId: z.string().uuid('Selecciona el deporte.'),
  categoryId: z.string().uuid('Selecciona la categoría.'),
  coachId: z.string().optional(),
  joinedAt: z.string().min(1, 'Selecciona la fecha de ingreso.'),
  ...
  guardianName: z.string().trim().min(2, 'Escribe el nombre del acudiente.').max(160),
  guardianRelationship: z.string().trim().min(2, 'Indica el parentesco.').max(80),
  guardianPhone: z.string().trim().min(7, 'Escribe un teléfono válido.').max(40),
...
export function AthleteFormPage() {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<FormValues>({
    ...
    defaultValues: {
      internalCode: generatedCode, documentType: 'IDENTITY_CARD', documentNumber: '', firstNames: '', lastNames: '',
      birthDate: '', sex: 'FEMALE', municipality: '', zone: 'URBAN', sportsProgramId: '', sportId: '', categoryId: '',
      coachId: '', joinedAt: new Date().toISOString().slice(0, 10), status: 'ACTIVE', schoolName: '', schoolGrade: '',
      schoolShift: 'Mañana', currentlyEnrolled: true, guardianName: '', guardianRelationship: '', guardianPhone: '', guardianEmail: '',
    },
  });
  ...
  async function submit(values: FormValues) {
    ...
    const input: AthleteInput = {
      id: athleteId, internalCode: values.internalCode, documentType: values.documentType,
      documentNumber: values.documentType === 'NONE' ? null : values.documentNumber || null,
      firstNames: values.firstNames, lastNames: values.lastNames, birthDate: values.birthDate, sex: values.sex,
      municipality: values.municipality, zone: values.zone, sportsProgramId: values.sportsProgramId, sportId: values.sportId,
      categoryId: values.categoryId, coachId: values.coachId || null, joinedAt: values.joinedAt, status: values.status,
      ...
      guardian: { name: values.guardianName, relationship: values.guardianRelationship, phone: values.guardianPhone, email: values.guardianEmail || null },
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-coral-600">Expediente único</p><h1 className="mt-1 font-display text-4xl text-pine-900">{id ? 'Editar deportista' : 'Nuevo deportista'}</h1></div>{!online && <span className="inline-flex items-center gap-2 rounded-full bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-800"><WifiOff size={15} /> Guardado sin conexión</span>}</div>
      ...
      <form className="mt-7 space-y-6" onSubmit={handleSubmit(submit)}>
        <FormSection title="Identificación" description="Datos básicos para reconocer al deportista.">
          <label><span className="text-sm font-semibold text-slate-700">Código interno *</span><input className={inputClass} {...register('internalCode')} /><FieldError message={errors.internalCode?.message} /></label>
          <label><span className="text-sm font-semibold text-slate-700">Tipo de documento *</span><select className={inputClass} {...register('documentType')}><option value="CIVIL_REGISTRY">Registro civil</option><option value="IDENTITY_CARD">Tarjeta de identidad</option><option value="PASSPORT">Pasaporte</option><option value="PERMIT">Permiso</option><option value="NONE">No tiene</option><option value="OTHER">Otro</option></select></label>
          ...
          <label><span className="text-sm font-semibold text-slate-700">Sexo *</span><select className={inputClass} {...register('sex')}><option value="FEMALE">Femenino</option><option value="MALE">Masculino</option><option value="INTERSEX">Intersexual</option><option value="OTHER">Otro</option><option value="PREFER_NOT_TO_SAY">Prefiere no responder</option></select></label>
          ...
          <label><span className="text-sm font-semibold text-slate-700">Zona *</span><select className={inputClass} {...register('zone')}><option value="URBAN">Urbana</option><option value="RURAL">Rural</option></select></label>
        </FormSection>
