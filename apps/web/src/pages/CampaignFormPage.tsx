...
export function CampaignFormPage() {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <div className="mt-4 rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm leading-6 text-emerald-800"><strong>Participación abierta por edad.</strong> El rango indicado en el instrumento es una orientación; puedes agregar deportistas de cualquier edad.</div>
    ...
    <section className="card mt-6 grid gap-5 p-5 md:grid-cols-2 md:p-7">
      ...
      <Field label="Instrumento"><select className="field" value={values.instrumentId} onChange={(event) => setValues({ ...values, instrumentId: event.target.value })}>{instruments.map((item) => <option key={item.id} value={item.id}>{item.name} v{item.version}{item.ageGroup ? ` · rango orientativo ${item.ageGroup}` : ''}</option>)}</select></Field>
