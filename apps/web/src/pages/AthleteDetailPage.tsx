    ...
    <div>
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_.8fr]">
        ...
        {can(PERMISSIONS.SOCIAL_RECORD_READ) && <aside className="space-y-6">
          ...
          <section className="card p-6 md:p-7"><p className="text-xs font-bold uppercase tracking-[.14em] text-coral-600">Trabajo Social</p><h2 className="mt-2 font-display text-3xl text-pine-900">Instrumentos</h2><div className={`mt-5 rounded-2xl p-5 ${athlete.hasSocialRecord ? 'bg-emerald-50' : 'bg-amber-50'}`}>{athlete.hasSocialRecord ? <><BookOpen className="text-emerald-700" /><p className="mt-3 font-semibold text-emerald-900">Ficha social registrada</p><p className="mt-1 text-xs text-emerald-700">Última actualización: {athlete.socialRecordUpdatedAt ? new Date(athlete.socialRecordUpdatedAt).toLocaleDateString('es-CO') : 'reciente'}</p></> : <><ShieldAlert className="text-amber-700" /><p className="mt-3 font-semibold text-amber-900">Instrumentos pendientes</p><p className="mt-1 text-xs leading-5 text-amber-700">Selecciona la ficha, entrevista o valoración que deseas aplicar.</p></>}</div><Link to={`/instrumentos?athleteId=${athlete.id}`} className="btn-primary mt-5 w-full"><ClipboardEdit size={18} /> Aplicar instrumento</Link></section>
